...
<template>
  ...
  <ContentBox>
    ...
    가장 근본적인 목적은 바로 재사용성 입니다. 이 그림에서도 보시다시피
    컴포넌트들이 조직적으로 모여서 하나의 어플리케이션으로 조립될 수 있습니다.
    이 다음 화면에서 보면 이 투두리스트의 리스트 하나하나가 바로 컴포넌트가
  ...
  <ContentTitle title="Vue.js와 MVVM 패턴" />
  <ContentBox>
    Vue.js에서 반응형 데이터는 <span>ref</span>,<span>reactive</span>를 사용하여
    적용할 수 있습니다. ref와 reactive를 활용해 데이터를 할당하면, Vue 자체의
    기능으로 Proxy객체로 감싸지면서 변화에 따른 감지가 가능해집니다. MVVM
    패턴이란, 소프트트웨어 아키텍처의 종류 중 하나인데요. Model, View Model,
    View 이렇게 세가지고 구성된 아키텍처를 의미합니다. 이 소프트웨어 아키텍처는
    소프트웨어 시스템의 기본 구조와 시스템을 만드는 프로그램의 설계도 같은
    것입니다.
  </ContentBox>

  <ContentBox>
    이런 개념을 함께하고 있는 Vue의 아키텍처에 대해 말씀드리겠습니다. 아까 잠깐
    말씀드렸다시피, MVVM 패턴은 ViewModel과 View, 그리고 Model로
    구성되어있습니다. 이 View는 저희가 보는 화면을 뜻합니다. 그리고 이 Model은
    Javascript 코드를 의미합니다. 그리고 이 ViewModel은 vue 인스턴스 입니다. 이
    ViewModel 그러니까 Vue.js는 우리가 보는 이 화면과, 우리가 작성하는
    Javascript 코드의 중간 매개체 역할을 해주고 있습니다.
  </ContentBox>
  <ContentBox>
    만약에 우리가 화면에서 버튼을 클릭해서 어떤 함수를 실행시켰을 때, 아마
    어떠한 data가 변경되었을 것입니다. 이 때 이 ViewModel은 사용자의 이벤트를
    감지하고 Javascript에게 이벤트가 실행되었음을 알려주게 됩니다. Javascript는
    이벤트 실행에 따라 변경된 데이터를 Javascript에 적용하고, 다시 이
    ViewModel은 Javascript에서 변경된 데이터를 우리가 보는 화면에
    적용시켜줍니다.
  </ContentBox>
  <ContentTitle mt="mt" title="Vue.js 핵심 기능 2 가지" />
  <ContentBox>
    ...
    데이터 바인딩이 가능합니다.
    <br /><br />
    두번째로 아까 잠시 말씀드렸던 반응성 데이터 입니다. 이 반응성 데이터는
    javascript의 Proxy 객체를 활용한 기능인데요, 반응성으로 동작할 수 있는
    원리에 대해서 간략히 말씀드리면, 반응형 데이터들은 할당과 동시에 Observe
    기능이 활성화 됩니다. 데이터가 변경되었을 때 내부의 Observe 기능으로 감지한
    변화에 따라, (46페이지 텍스트 변경) 해당 데이터를 사용하는 요소를 찾아서
    반영시키게 됩니다.
    <br /><br />
    이 반응성을 적용하는 방법에는 Vue의 ref와 reactive함수를 활용해서 사용할 수
    있습니다. ref는 원시자료형, 객체, 배열을 모두 사용할 수 있고, reactive는
    객체나 배열에 한해서만 사용할 수 있습니다.
  </ContentBox>
</template>
